...
*/	
	$('#datagrid').datagrid({
		...
		columns:[[
				...
				{field:'appAdName',title:'应用广告名称',width:150,align:'center'},
				{field:'adStatusName',title:'应用广告状态',width:150,align:'center'},
...
function submitAddAd(operatype)
{
	$('#ff').form('submit',{
		...
		onSubmit:function(param){
			...
			if($('#ff').form('enableValidation').form('validate')&&appList.keys.length>0&&areaList.keys.length>0 &&imgOrwordFlag)
				{
					flag = true;
				}
			else
...
function submitUpdateAd(operatype)
{
	$('#ffUpdate').form('submit',{
		...
		onSubmit:function(param){
			...
			if($('#ffUpdate').form('enableValidation').form('validate') &&appList.keys.length>0 &&areaList.keys.length>0 && imgOrwordFlag)
				{
					flag = true;
				}
			else
